...
describe('QuoteRequestService', () => {
  ...
  it('should upload the attachment and insert its metadata', async () => {
    ...
    expect(uploadMock).toHaveBeenCalledWith(expect.stringMatching(/\/menu-gerson\.pdf$/), file, {
  ...
  it('should throw an error and not insert when the attachment upload fails', async () => {
    ...
    await expect(service.createQuoteRequest(quoteRequest, file)).rejects.toThrow('Upload failed');
  ...
  it('should throw an error when the database insert fails', async () => {
    ...
    await expect(service.createQuoteRequest(quoteRequest, null)).rejects.toThrow(
